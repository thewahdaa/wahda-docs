...
Every project on The Wahda Cloud gets its own **private network fabric** — networks, subnets, routers, security groups — with an on-ramp to the public internet through floating IPs and, when you need it, a managed **Octavia load balancer** or an **IPsec VPN** into another site.
...
| **[Load balancer →](/networking/load-balancer)** | A managed L4/L7 load balancer (Octavia) that fronts multiple backends. | You have more than one VM answering the same traffic and want a single entry point with health checks. |
